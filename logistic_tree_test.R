data = read.csv("cleaned-ks-data.csv")
data = data[data$state == 'successful' | data$state == 'failed', ]
data$state = factor(data$state)
train = sample(nrow(data), 10000)
glm.fits=glm(state~usd_goal_real+category+duration, data=data , family = binomial, subset = train)
...
library(tree)
tree.state = tree(state~main_category+usd_goal_real+duration , data )
plot(tree.state)
